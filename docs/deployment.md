...
**Don't deploy unless the user asks** — they control timing.
**No QA by default:** when the user asks to push / ship / deploy, commit and push only. Run local QA,
smoke tests, Playwright checks, or post-deploy verification only when the user explicitly asks.
...
fly deploy -a backend-lively-hill-7043     # run in BACKGROUND — it's slow
```
...
```
When requested, use **playwright MCP**: navigate to `https://www.axliner.com` (give Vercel a moment; re-check if the
old build is still served) → `browser_take_screenshot` → `browser_console_messages` for runtime errors
→ spot-check the change actually landed (`browser_evaluate` for visual fixes). Clean up artifacts.
For DB-linked changes, confirm advisors are clean (`get_advisors`). **Report honestly** if the smoke
test fails or the build is still propagating — don't claim "shipped and verified" until you've seen green.
