...
type VideoPlaceholderProps = {
  /** When set, the real Screen Studio recording plays here (mp4/webm). */
  src?: string
  /** Optional still frame shown before the video paints. */
  poster?: string
  caption?: string
  className?: string
}

export function VideoPlaceholder({ src, poster, caption, className }: VideoPlaceholderProps) {
  return (
    <div className={cn("group", className)}>
      {/* Frame */}
      <div className="relative aspect-video w-full overflow-hidden rounded-xl">
        {src ? (
          /* Real recording — autoplay, looping, muted (landing-page friendly) */
          <video
