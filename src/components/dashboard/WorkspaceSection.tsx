...
const toneAccent: Record<SectionTone, string> = {
  default: "",
  active: "border-[var(--workspace-selection-border)]",
...
export function WorkspaceSection({
  ...
  const heading = (
    <div className={cn("flex min-w-0 flex-1 items-start", compact ? "gap-2.5" : "gap-3")}>
      {icon ? (
        <span
          className={cn(
            "inline-flex shrink-0 items-center justify-center rounded-lg bg-[var(--workspace-blue-soft)] text-[var(--workspace-primary)] [&_svg]:text-[var(--workspace-primary)]",
      ...
      <div className="min-w-0 flex-1">
        <div className="flex min-w-0 items-center gap-2">
          {step !== undefined && step !== null ? (
            <span
              className={cn(
                "inline-flex shrink-0 items-center justify-center rounded-md bg-[var(--workspace-blue-soft)] font-mono font-semibold tabular-nums text-[var(--workspace-primary)] ring-1 ring-inset ring-[var(--workspace-selection-border)]",
        ...
        {hint ? (
          <p
            className={cn(
              "mt-1 max-w-[52ch] text-pretty text-[12px] leading-5 text-[var(--workspace-muted)]",
      ...
      {collapsible ? (
        <ChevronDown
          className={cn(
            "ml-1 size-4 shrink-0 text-[var(--workspace-muted)] transition-transform duration-200",
  ...
  return (
    <section
      id={id}
      className={cn(
        "relative overflow-hidden rounded-lg border border-[var(--workspace-border)] bg-[var(--workspace-popout-bg)] shadow-none",
        toneAccent[tone],
        className,
      )}
    >
      ...
      <AnimatePresence initial={false}>
        {isOpen ? (
          ...
          >
            <div className={cn(compact ? "px-3.5 py-3.5 sm:px-4" : "px-4 py-4 sm:px-5", contentClassName)}>
              {children}
            </div>
          </motion.div>
        ) : null}
