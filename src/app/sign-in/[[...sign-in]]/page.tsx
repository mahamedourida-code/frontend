...
export default function SignInPage() {
  ...
  const onSignInSubmit = async (data: SignInInput) => {
    ...
    try {
      ...
      router.push('/dashboard/client') // Redirect to client instead of upload-type
      router.refresh()
    } catch (err: any) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/auth/callback?next=/dashboard/client`, // Redirect to client
      },
    })
