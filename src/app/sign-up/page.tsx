...
function SignUpContent() {
  ...
  const handleEmailSignUp = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await signUpWithEmailVerification(email.trim(), password, {
  ...
  return (
    <main className="ax-page-bg min-h-screen px-4 py-8">
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-6xl items-center justify-center">
        <Card className="w-full max-w-[460px] overflow-hidden rounded-[28px] border border-[#eadfff] bg-white/72 shadow-[0_26px_90px_rgba(68,31,132,0.12)] backdrop-blur-xl">
          <CardContent className="p-6 sm:p-8">
            ...
            <div className="mb-7 text-center">
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                Start converting images and keep your batch history.
              </p>
            </div>

            {submittedEmail ? (
              <div className="rounded-[24px] border border-[#eadfff] bg-white/60 p-5 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.82)]">
                <h2 className="text-lg font-black text-foreground">Check your email</h2>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">
                  We sent a confirmation link to <span className="font-semibold text-foreground">{submittedEmail}</span>.
                  Open it to finish creating your AxLiner account.
                </p>
                <Button
                  className="mt-5 h-11 rounded-2xl bg-[#2f165e] px-5 font-bold text-white hover:bg-[#24114a]"
                  onClick={() => router.push(`/sign-in?next=${encodeURIComponent(nextPath)}`)}
                >
                  Back to sign in
                </Button>
              </div>
            ...
            <p className="mt-7 text-center text-sm text-muted-foreground">
              ...
              <Link href={`/sign-in?next=${encodeURIComponent(nextPath)}`} className="font-bold text-[#2f165e] hover:underline">
                Sign in
              </Link>
            </p>
              </>
