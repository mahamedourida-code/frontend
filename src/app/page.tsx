...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // 1. Floating Cards Animation - Continuous subtle movement
    const cards = document.querySelectorAll('[data-animate="stagger"]');
    if (cards.length > 0) {
      anime({
        targets: cards,
        translateY: [
          { value: -8, duration: 2000, easing: 'easeInOutSine' },
    ...
    const createMorphingShapes = () => {
      ...
      if (heroSection) {
        ...
        blob1.className = 'absolute top-20 left-10 w-64 h-64 bg-primary/5 rounded-full blur-3xl';
        blob2.className = 'absolute bottom-20 right-10 w-96 h-96 bg-primary/5 rounded-full blur-3xl';
    ...
    // 3. Stagger Text Reveal - Letter by letter animation
    const headlines = document.querySelectorAll('[data-animate="headline"]');
    headlines.forEach((headline) => {
      const text = headline.querySelector('h2');
      if (text) {
    ...
    // Apply to metric numbers
    const metricElements = document.querySelectorAll('.text-2xl.font-bold.text-primary');
    metricElements.forEach((el) => {
      ...
      if (text.includes('%')) {
        ...
        const num = parseFloat(text);
        if (!isNaN(num)) animateCounter(el, num, 's/page');
      }
    });

    // 5. Magnetic Button Effect
    const buttons = document.querySelectorAll('button, .button, a[href="/sign-in"]');
    ...
    // 6. Scroll-triggered pulse animation for badges
    const badges = document.querySelectorAll('.inline-flex.items-center.gap-2.px-3');
    ...
    // 7. Particle effect on hover for main CTA
    const mainCTA = document.querySelector('button[class*="bg-primary"]');
    if (mainCTA) {
      mainCTA.addEventListener('mouseenter', () => {
        anime({
          targets: mainCTA,
          boxShadow: [
            { value: '0 0 0 0 rgba(34, 197, 94, 0.4)' },
            { value: '0 0 20px 10px rgba(34, 197, 94, 0)' }
          ],
          duration: 1000,
          easing: 'easeOutExpo'
        });
    ...
    // 8. Wave effect for section separators
    const separators = document.querySelectorAll('.border-t, .border-b');
